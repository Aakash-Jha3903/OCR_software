...
# Force CPU usage (safe for machines with no GPU or mismatched CUDA)
# Use paddle.is_compiled_with_cuda() to detect if paddle was built with CUDA.
use_gpu = False
try:
    use_gpu = paddle.is_compiled_with_cuda() and False  # keep False to force CPU
except Exception:
    use_gpu = False

ocr = PaddleOCR(use_angle_cls=True, lang='en', use_gpu=use_gpu)  # use_gpu=False
