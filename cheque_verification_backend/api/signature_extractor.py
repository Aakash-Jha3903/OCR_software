...
def extract_signature_flexible(img, debug=False):
    # Check if img is None
    if img is None:
        raise ValueError("Image is empty or invalid.")
    
    img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  # Work directly with the image array
    ...
    if not candidates:
        ...
        if debug:
            cv2.imshow("Fallback Signature", crop_fallback)
            cv2.waitKey(0)
